    ...
    def get(self, request, user_id):
        try:
            ...
            if not weight or not height:
                return Response({"error":"mainpage정보 호출 실패, 유저 정보 필요"}, status=409)

            if UserTest_Result == None:
                return Response({"error":"mainpage정보(운동) 호출 실패, 체력평가 결과 필요"}, status=409) 
...
class WorkoutGraphView(APIView):
    ...
    def get(self, request, workout, user_id):
        try:
            ...
            return Response({"error":"운동 그래프 정보 호출 실패."}, status=400)
#월 기록 호출
class MonthHistoryView(APIView):
    ...
    def get(self, request, year_month, user_id):
        try:
            ...
            return Response({"error":"월 기록 호출 실패."}, status=400)
...
class DayHistoryView(APIView):
    ...
    def get(self, request, date, user_id):
        try:
            ...
            return Response({"error":"해당일 기록이 존재하지 않습니다."}, status=409)
...
class DayHistoryWorkoutInfoView(APIView):
    ...
    def get(self, request, date, workout, user_id):
        try:
            ...
            return Response({"error":"당일 운동 정보가 존재하지 않습니다."}, status=409)